        ...
        function createToastContainer() {
            ...
            container.className = 'fixed top-4 left-1/2 -translate-x-1/2 z-[100] space-y-2 w-full max-w-md px-4 pointer-events-none';
                            ...
                            <div id="hw-fields-${job.id}" class="hidden space-y-3">
                                <div class="grid grid-cols-2 gap-2">
                                    <div class="space-y-1">
                                        ...
                                        <select name="device_name" onchange="window.showDevicePrice(this.value, '${job.id}')" class="w-full bg-black/50 border border-white/10 rounded-xl px-2.5 py-2 text-xs text-white focus:outline-none focus:border-indigo-500">
                                            <option value="">— Choose stock device —</option>
                                            ${(() => {
                                                try {
                                                    const catalog = JSON.parse(localStorage.getItem('cached_catalog') || '[]');
                                                    return catalog.map(c => `<option value="${c.item_code}">${c.item_name} (${c.stock_qty || 0} in stock)</option>`).join('');
                                                } catch(e) {
                                                    return '';
                                                }
                                            })()}
        ...
        function insertWorkstationRow(jobId, deviceId, osUpdated, tempCleaned, smartStatus) {
            ...
            tr.innerHTML = `
                ...
                <td class="py-2.5 text-center">
                    <button type="button" onclick="removeWorkstationRow(this)" class="w-6 h-6 rounded-lg bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 transition-all flex items-center justify-center font-bold text-[10px]">✕</button>
                </td>
            `;
