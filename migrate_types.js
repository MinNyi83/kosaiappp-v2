...
function traverseDir(dir) {
  ...
  for (const file of files) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      content = content.replace(/function register\(router, env\)/g, 'function register(router: any, env: any)');
      
      // Replace route signatures: router.post("/api/...", async (request) => 
      content = content.replace(/async \((request)\) =>/g, 'async (request: any) =>');
      content = content.replace(/async \((request, params)\) =>/g, 'async (request: any, params: any) =>');
