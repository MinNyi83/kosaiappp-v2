...
async function initializeAdminDesk() {
  ...
  workspace.innerHTML =
    '<div class="flex justify-center items-center h-64"><div class="animate-pulse text-amber-500 font-bold uppercase text-xs tracking-widest">Loading dispatch components...</div></div>';
...
function initSearchableSelects() {
  ...
  makeSearchableAll(
    [
      ...
      '#edit-ticket-tech',
      '[name="service_type"]',
      '[name="edit-ticket-service-type"]',
      '[name="edit-ticket-status"]',
      '#ticket-filter-domain',
    ],
    document,
    amberOpts
  );
  ...
  makeSearchableAll(
    [
      ...
      '[name="batch-item-code"]',
      '[name="catalog-item-category"]',
      '[name="catalog-sub-category"]',
      '[name="catalog-brand"]',
      '[name="catalog-um"]',
      '[name="price-update-item-code"]',
      '#new-subcat-parent',
    ],
    document,
    amberOpts
  ...
  // AMC / Clients view
  makeSearchableAll(
    ['#client-status-dropdown', '#client-sort-dropdown', '[name="amc_status"]'],
    document,
    amberOpts
  );
  ...
  makeSearchableAll(
    ['#new-user-role', '#edit-tech-role', '[name="user-filter-role"]'],
    document,
    amberOpts
  );
  ...
  makeSearchableAll(
    ['[name="cash-type"]', '[name="cash-currency"]', '[name="cash-batch-id"]'],
    document,
    amberOpts
  );
...
window.refreshSearchableSelect = function (elOrId) {
  ...
  if (el._ss) {
    ...
    window.makeSearchable(el, { accentColor: '#f59e0b' });
  }
};
...
function loadHQConfig() {
  ...
  const baseUrl = document.getElementById('api-base')
  ...
  if (editClientSearch) {
    editClientSearch.addEventListener('input', (e) => {
      ...
      if (selectedClient) {
        ...
        if (editClientPhone) {
          editClientPhone.required = false;
        }
        if (editClientAddress) {
          editClientAddress.required = false;
        }
      } else {
        editClientIdHidden.value = '__NEW__';
        if (editNewClientFields) editNewClientFields.classList.remove('hidden');
        if (editClientPhone) {
          editClientPhone.required = true;
        }
        if (editClientAddress) {
          editClientAddress.required = true;
        }
      }
    });
...
async function populateReports() {
  ...
  try {
    ...
    const totalTickets = jobsArray.length;
    ...
    document.getElementById('report-usd-safe').textContent =
      `$${(safe.usd_balance || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
    document.getElementById('report-mmk-safe').textContent =
      `${(safe.mmk_balance || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} Ks`;
    const amcCounts = { Active: 0, Inactive: 0, Expired: 0, 'No AMC': 0, Individual: 0 };
    if (lookups && Array.isArray(lookups.clients)) {
    ...
    if (lookups && Array.isArray(lookups.technicians)) {
      lookups.technicians.forEach((t) => {
        techLoad[t.id] = { name: t.name, load: 0, activeTickets: 0 };
      });
    }

    jobsArray.forEach((j) => {
      if (j.technician_id && techLoad[j.technician_id]) {
        techLoad[j.technician_id].load++;
        if (j.status !== 'Completed') {
          techLoad[j.technician_id].activeTickets++;
        }
      }
    });
  ...
  if (subCatEl) {
    subCatEl.innerHTML =
      ...
      masterSubCategories
        .map(
          (s) =>
            `<option value="${s.name}">${s.name}${s.category_name ? ' (' + s.category_name + ')' : ''}</option>`
        )
        .join('');
  }
...
function renderMasterLists() {
  ...
  if (catBody) {
    if (masterCategories.length === 0) {
      catBody.innerHTML =
        '<tr><td colspan="3" class="px-3 py-6 text-center text-[11px] text-slate-600">No categories yet</td></tr>';
    } else {
      catBody.innerHTML = masterCategories
        .map(
          (c, i) => `
                        <tr style="border-bottom: 1px solid rgba(255,255,255,0.04);">
                            <td class="px-3 py-2 text-[10px] text-slate-400 font-mono">${c.code || '—'}</td>
  ...
  if (subBody) {
    if (masterSubCategories.length === 0) {
      subBody.innerHTML =
        '<tr><td colspan="4" class="px-3 py-6 text-center text-[11px] text-slate-600">No sub-categories yet</td></tr>';
    } else {
      subBody.innerHTML = masterSubCategories
        .map(
          (s) => `
                        <tr style="border-bottom: 1px solid rgba(255,255,255,0.04);">
                            <td class="px-3 py-2 text-[10px] text-slate-400 font-mono">${s.code || '—'}</td>
                            <td class="px-3 py-2 text-[11px] text-white font-medium">${s.name}</td>
                            <td class="px-3 py-2 text-[9px] text-slate-500">${s.category_name || '—'}</td>
                            <td class="px-3 py-2 flex items-center gap-2">
  ...
  if (brandBody) {
    if (masterBrands.length === 0) {
      brandBody.innerHTML =
        '<tr><td colspan="3" class="px-3 py-6 text-center text-[11px] text-slate-600">No brands yet</td></tr>';
    } else {
      brandBody.innerHTML = masterBrands
        .map(
          (b, i) => `
                        <tr style="border-bottom: 1px solid rgba(255,255,255,0.04);">
                            <td class="px-3 py-2 text-[10px] text-slate-400 font-mono">${b.code || '—'}</td>
  ...
  if (unitsBody) {
    if (masterUnits.length === 0) {
      unitsBody.innerHTML =
        '<tr><td colspan="3" class="px-3 py-6 text-center text-[11px] text-slate-600">No units yet (defaults: pcs, meter, pack, box...)</td></tr>';
    } else {
      unitsBody.innerHTML = masterUnits
        .map(
          (u) => `
                        <tr style="border-bottom: 1px solid rgba(255,255,255,0.04);">
...
window.switchInvModule = function (module) {
  ...
  // Reset register form if entering add-model normally
  if (module === 'add-model') {
    const titleEl = document.getElementById('add-model-title');
    if (titleEl) titleEl.innerText = 'Register Device Model';
    const form = document.getElementById('catalog-add-form');
    if (form) form.reset();
    const codeInput = document.getElementById('catalog-item-code');
    if (codeInput) {
...
window.editInventoryItem = function (item_code) {
  ...
  // Set title
  const titleEl = document.getElementById('add-model-title');
  if (titleEl) titleEl.innerText = 'Edit Device Model';

  // Populate fields
  const nameInput = document.getElementById('catalog-item-name');
  ...
  const catSelect = document.getElementById('catalog-item-category');
  if (catSelect) {
    catSelect.value = item.category || '';
    window.loadSubCategoriesForForm(item.category || '');
  }

  setTimeout(() => {
    const subCatSelect = document.getElementById('catalog-sub-category');
    if (subCatSelect) subCatSelect.value = item.sub_category_id || '';

    const brandSelect = document.getElementById('catalog-brand');
    if (brandSelect) brandSelect.value = item.brand_id || '';

    const umSelect = document.getElementById('catalog-um');
    if (umSelect) umSelect.value = item.stocking_um || '';
  }, 100);
};

// ── Master Data submit handlers ──
...
window.loadSubCategoriesForForm = function (categoryName) {
  const subCatEl = document.getElementById('catalog-sub-category');
  if (!subCatEl) return;
  const filtered = masterSubCategories.filter(
    (s) => !categoryName || s.category_name === categoryName
  );
  subCatEl.innerHTML =
    '<option value="">— Select —</option>' +
    ...
    for (const r of rows) {
      try {
        ...
        const res = await fetch(`${baseUrl}/api/admin/inventory/catalog/price`, {
          ...
          headers: headers,
          body: JSON.stringify({
            item_code: r['SKU'] || r['item_code'],
            unit_price: parseFloat(r['USD Price'] || r['unit_price']) || 0,
            unit_price_mmk: parseInt(r['MMK Price'] || r['unit_price_mmk']) || 0,
          }),
        });
        res.ok ? ok++ : fail++;
      } catch {
    ...
    for (const r of rows) {
      try {
        ...
        const res = await fetch(`${baseUrl}/api/admin/inventory/add`, {
          ...
          headers: headers,
          body: JSON.stringify({
            item_code: (r['SKU'] || r['item_code'] || '').toUpperCase(),
            item_name: r['Model Name'] || r['item_name'] || '',
            category: r['Category'] || r['category'] || '',
            sub_category_id: r['Sub-Cat'] || '',
            brand_id: r['Brand'] || '',
            stocking_um: r['U/M'] || 'pcs',
            stock_qty: 0,
            unit_price: 0,
            unit_price_mmk: 0,
          }),
...
window.importBatchExcel = function (input) {
  ...
  const overwrite = confirm(
    'Do you want to overwrite existing batches if duplicate Batch Codes are found?'
  );
  loadXLSX(async () => {
    const data = await file.arrayBuffer();
    const wb = XLSX.read(data);
    ...
    for (const r of rows) {
      try {
        ...
        const res = await fetch(`${baseUrl}/api/admin/inventory/batches/create`, {
          ...
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
          body: JSON.stringify({
            batch_code: String(r['Batch Code'] || r['batch_code'] || '')
              .toUpperCase()
              .trim(),
            item_code: String(r['SKU'] || r['item_code'] || '')
              .toUpperCase()
              .trim(),
                    ...
                    <tr id="serials-row-${idx}" class="hidden">
                        <td colspan="11" class="px-5 py-3" style="background: rgba(0,0,0,0.25); border-bottom: 1px solid rgba(16,185,129,0.1);">
                            <div class="flex items-center gap-2 mb-2">
                                ...
                                <p class="text-[8px] font-black text-emerald-600 uppercase tracking-[0.15em]">${isSerial ? 'Serial Numbers' : 'Quantity batch'} · ${b.batch_code} · ${availableUnits}/${totalUnits} Available</p>
                            </div>
...
function renderSalesPricing() {
  ...
  if (activeCatalogList.length === 0) {
    if (pricingBody)
      pricingBody.innerHTML =
        '<tr><td colspan="8" class="px-4 py-8 text-center text-slate-600 text-[11px]">No models in catalog yet.</td></tr>';
    if (catalogBody)
  ...
  let filterSelectHtml = '<option value="">— All Models —</option>';
  let pricingRowsHtml = '';
  let catalogRowsHtml = '';
...
window.filterBatchTable = function () {
  ...
  rows.forEach((row) => {
    if (row.id && row.id.startsWith('serials-row')) {
      row.style.display =
        currentParentVisible && row.classList.contains('expanded-visible') ? '' : 'none';
...
window.posEditBatchPrompt = async function (batchCode, currentPrice, currentSupplier) {
  const priceVal = prompt(
    `Edit Stock Batch "${batchCode}"\n\nEnter new Buying Price / unit (USD):`,
    currentPrice
  );
  if (priceVal === null) return; // Cancelled

  const supplierVal = prompt(
...
function renderCashTable() {
  ...
  sliced.forEach((tx) => {
    ...
    const batchBadge = tx.linked_batch
      ? `<span class="bg-indigo-500/15 text-indigo-400 px-1.5 py-0.5 rounded font-mono text-[9px] font-bold">Batch: ${tx.linked_batch}</span>`
      : '';
...
async function loadTechniciansData() {
  ...
  try {
    ...
    techs.forEach((t) => {
      ...
      let roleOptions = '';
      const rList =
        window.globalRolesList && window.globalRolesList.length > 0
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* CR80 card: 85.6 Ã� 53.98 mm at 96 DPI → px ratio */
...
window.submitEditTechnician = async function (e) {
  ...
  const telegram_username =
    document.getElementById('edit-tech-telegram-username')?.value?.trim() || '';
...
window.deleteTechnician = async function (id) {
  if (
    !confirm(
      'Are you sure you want to delete this technician? If they have completed jobs, they will be deactivated instead.'
    )
  )
    return;
...
window.openTechHistoryModal = async function (techId, techName) {
  ...
  listContainer.innerHTML =
    '<p class="text-xs text-slate-500 py-4 text-center">Pulling service logs...</p>';
  ...
  try {
    ...
    if (techHistoryModalJobs.length === 0) {
      listContainer.innerHTML =
        '<p class="text-xs text-slate-500 py-4 text-center">No assigned tickets found for this technician.</p>';
      return;
    }
    ...
    listContainer.innerHTML =
      '<p class="text-xs text-rose-400 py-4 text-center">Failed to load service records.</p>';
    console.error(err);
  }
};
...
window.openEditTicketModal = function (
  ...
  document.getElementById('edit-ticket-maps-url').value =
    mapsUrl === 'undefined' || !mapsUrl ? '' : mapsUrl;
  document.getElementById('edit-job-lat').value =
    arrivalLat === 'undefined' || !arrivalLat ? '' : arrivalLat;
  document.getElementById('edit-job-lng').value =
    arrivalLng === 'undefined' || !arrivalLng ? '' : arrivalLng;
...
window.submitEditTicket = async function (e) {
  ...
  try {
    if (client_id === '__NEW__' || !client_id) {
      ...
      const existing = window.allClientsList?.find(
        (c) => c.company_name.toLowerCase() === typedName.toLowerCase()
      );
      if (existing) {
...
function setMapTheme(theme) {
  ...
  if (btnDark)
    btnDark.className =
      theme === 'dark'
        ? 'bg-amber-500/10 border border-amber-500/30 text-amber-500 py-2 rounded-lg'
        : 'bg-white/5 border border-white/10 text-slate-400 hover:text-white py-2 rounded-lg';
  if (btnLight)
    btnLight.className =
      theme === 'light'
        ? 'bg-amber-500/10 border border-amber-500/30 text-amber-500 py-2 rounded-lg'
...
function renderAnalytics(jobs) {
  if (!Array.isArray(jobs)) {
    console.warn("Analytics skipped: 'jobs' data is missing or invalid.");
    return;
  }
...
window.setClientDirectoryTab = function (tab) {
  ...
  if (tab === 'corporate') {
    btnCorp.className =
      'text-xs font-bold px-3 py-1.5 rounded-lg transition-all text-white border border-amber-500/30 bg-amber-500/10';
    btnInd.className =
      'text-xs font-bold px-3 py-1.5 rounded-lg transition-all text-slate-400 border border-transparent hover:text-white';
...
function renderClientsTable(list) {
  ...
  if (list.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="5" class="py-4 text-center text-slate-600">No clients match this filter criteria.</td></tr>';
    return;
  }
  let rowsHtml = '';
  list.forEach((c) => {
...
window.openEditClient = function (id) {
  ...
  document.getElementById('client-form-title').innerHTML =
    '<span class="text-amber-500">✏️</span> Edit Client Profile';
...
window.resetClientForm = function () {
  ...
  document.getElementById('client-form-title').innerHTML =
    '<span class="text-amber-500">🏢</span> Provision Client Profile';
...
window.openClientHistoryModal = async function (clientId, companyName) {
  ...
  try {
    ...
    if (historyModalJobs.length === 0) {
      listContainer.innerHTML =
        '<p class="text-xs text-slate-500 py-4 text-center">No service records registered for this client.</p>';
      return;
    }
  ...
  try {
    ...
    if (reportType === 'tech-performance') {
      ...
      cashTransactions.forEach((t) => {
        ...
        if (t.job_id) {
          ...
          if (job && clientStats[job.client_id]) {
            const val =
              parseFloat(t.amount) *
              (t.primary_currency === 'USD' ? 1 : 1 / parseFloat(t.exchange_rate));
      ...
      filteredClients.forEach((c) => {
        ...
        const isNearExpiry =
          typeof remainingDays === 'number' && remainingDays <= 30 && remainingDays > 0;
...
function initCopilotMap() {
  ...
  L.marker([hq.lat, hq.lng])
    .addTo(copilotMap)
    .bindPopup('🏫 <strong>AwesomeMyanmar Office (Start/End)</strong>')
    .openPopup();
...
async function optimizeTechRoute() {
  ...
  routeContainer.innerHTML =
    '<p class="text-amber-500 animate-pulse">Running Travelling Salesperson optimization...</p>';
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data.route.length === 0) {
        routeContainer.innerHTML =
          '<p class="text-slate-500 italic">No active dispatches found for this engineer today.</p>';
        return;
      }
      ...
      data.route.forEach((stop, index) => {
        ...
        const m = L.marker([stop.lat, stop.lng])
          .addTo(copilotMap)
          .bindPopup(
            `<strong>Stop #${index + 1}: ${stop.company_name}</strong><br>${stop.address}`
          );
...
window.loadPosData = async function () {
      ...
      if (e.key === 'Enter') {
        ...
        try {
          const res = await fetch(
            `${baseUrl}/api/pos/resolve-serial?serial=${encodeURIComponent(val)}`,
            {
              headers: { Authorization: `Bearer ${token}` },
            }
          );
  ...
  try {
    ...
    if (stockRes.ok) {
      ...
      if (catSelect) {
        const uniqueCats = [...new Set(window.posCatalog.map((item) => item.category))].filter(
          Boolean
        );
        catSelect.innerHTML =
    ...
    const jobsRes = await fetch(`${baseUrl}/api/admin/cash/transactions`, {
      // using generic auth
      headers: { Authorization: `Bearer ${token}` },
    });
    ...
    if (jobsSelect) {
      ...
      const recordsQuery = await fetch(`${baseUrl}/api/admin/ai/transcribe`, {
        method: 'POST',
      }).catch(() => null); // mock trigger

      // Let's fetch jobs
      const fetchRecords = await fetch(`${baseUrl}/api/jobs/receipt?job_id=`, {
        headers: { Authorization: `Bearer ${token}` },
      }).catch(() => null);

      // Fetch list of tickets from tickets database
      const tListRes = await fetch(`${baseUrl}/api/admin/cash/transactions`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      // Let's fetch the list from general dashboard list
      const list = window.globalJobsList || [];
      jobsSelect.innerHTML =
        '<option value="">-- Link Ticket / Job ID --</option>' +
        list
          .map(
            (j) => `<option value="${j.id}">${j.id} - ${j.company_name || 'Individual'}</option>`
          )
          .join('');
    }
...
window.posLoadCreditsLedger = async function () {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (credits.length === 0) {
        tbody.innerHTML =
          '<tr><td colspan="8" class="px-4 py-10 text-center text-[11px] text-slate-600">No outstanding credit records found.</td></tr>';
        return;
      }
...
window.posRecallCreditInvoice = function (clientId, invoiceId, creditAmount) {
  ...
  alert(
    `Recalled Credit Invoice ${invoiceId} for Client: ${matched ? matched.company_name : clientId}. Record payment of $${creditAmount} to clear.`
  );
};
  ...
  filtered.forEach((item, idx) => {
    ...
    const catColorMap = {
      ...
      'Network Cables': {
        ...
        bdr: 'rgba(59,130,246,0.28)',
        txt: '#60a5fa',
      },
      'Security IP Cams': {
        bg: 'rgba(239,68,68,0.12)',
        bdr: 'rgba(239,68,68,0.28)',
        txt: '#f87171',
      },
      'Spare Hardware Parts': {
        bg: 'rgba(168,85,247,0.12)',
        bdr: 'rgba(168,85,247,0.28)',
        txt: '#c084fc',
      },
    };
    ...
    const icon =
      catIconMap[item.category] ||
      `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" class="w-4 h-4"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="12" y1="17" x2="12" y2="21"/><line x1="8" y1="21" x2="16" y2="21"/></svg>`;
...
window.removeFromCart = function (itemCode) {
  ...
  if (inCart && inCart.serials) {
    window.scannedSerialsList = (window.scannedSerialsList || []).filter(
      (s) => !inCart.serials.includes(s)
    );
  }
...
window.calculatePosTotals = function () {
  ...
  const customRate =
    parseFloat(document.getElementById('pos-custom-exchange-rate')?.value) || standardRate;
  ...
  if (subtotalText)
    subtotalText.textContent = `$${subtotal.toFixed(2)} / ${(subtotal * activeRate).toLocaleString()} Ks`;
...
window.executePosCheckout = async function () {
  ...
  if (
    !matchedClient &&
    (clientInput.toLowerCase().includes('individual') ||
      clientInput.toLowerCase().includes('walk-in'))
  ) {
    matchedClient = {
      id: 'Individual',
      company_name: 'Individual Clients / Walk-in',
  ...
  const custom_exchange_rate =
    parseFloat(document.getElementById('pos-custom-exchange-rate')?.value) || exchange_rate;
...
window.printInvoicePdf = function (tx) {
  ...
  if (cfg.show_payment) {
    ...
    y += isThermal ? 3.5 : 4.5;
    doc.text(
      `- Method A: ${tx.payment_method_a || 'N/A'} - Paid: ${tx.paid_amount_a || 0}${activeCurrencySymbol}`,
      margin,
      y
    );
    y += isThermal ? 3.5 : 4.5;
    doc.text(
      `- Method B: ${tx.payment_method_b || 'N/A'} - Paid: ${tx.paid_amount_b || 0}${activeCurrencySymbol}`,
      margin,
      y
    );
    ...
    if (tx.credit_amount > 0) {
      ...
      doc.setFont('helvetica', 'bold');
      doc.text(
        `- Credit Balance (Pay Later): ${tx.credit_amount}${activeCurrencySymbol}`,
        margin,
        y
      );
      doc.setFont('helvetica', 'normal');
...
window.loadPosSalesHistory = async function () {
  ...
  try {
    ...
    if (sales.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="6" class="py-4 text-center text-slate-600">No POS transactions booked yet.</td></tr>';
      return;
    }
...
window.reprintInvoicePdf = function (id, notes, amount, currency, rate) {
  ...
  const clientPart = parsedNotes[0]
    ? parsedNotes[0].replace('[POS CHECKOUT] Client:', '').trim()
    : 'Walk-in Client';
  ...
  const cartItems = itemsPart.split(',').map((part) => {
    ...
    if (match) {
      return {
        ...
        unit_price:
          currency === 'USD' ? amount / parseInt(match[2]) : amount / rate / parseInt(match[2]),
      };
    }
    return {
      item_name: part.trim(),
      qty: 1,
      unit_price: currency === 'USD' ? amount : amount / rate,
    };
  });
...
// ─────────────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────────────
// 🎨 RECEIPT PDF DESIGN BUILDER FUNCTIONS
// ─────────────────────────────────────────────────────────────────────
...
window.updatePdfPreview = function () {
  ...
  const subtitle =
    document.getElementById('pdf-cfg-subtitle')?.value || 'POINT OF SALE INVOICE RECEIPT';
  ...
  if (document.getElementById('pdf-cfg-color-hex'))
    document.getElementById('pdf-cfg-color-hex').value = color;
  if (document.getElementById('pdf-cfg-accent-hex'))
    document.getElementById('pdf-cfg-accent-hex').value = accent;

  // Sync Paper Size Badge in UI
  const sizeBadge = document.getElementById('pdf-preview-size-badge');
  if (sizeBadge)
    sizeBadge.textContent = `${paper} (${orientation === 'portrait' ? 'Port' : 'Land'})`;
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data.value) {
        ...
        if (document.getElementById('pdf-cfg-company'))
          document.getElementById('pdf-cfg-company').value = config.company || '';
        if (document.getElementById('pdf-cfg-subtitle'))
          document.getElementById('pdf-cfg-subtitle').value = config.subtitle || '';
        if (document.getElementById('pdf-cfg-color'))
          document.getElementById('pdf-cfg-color').value = config.color || '#0f172a';
        if (document.getElementById('pdf-cfg-accent'))
          document.getElementById('pdf-cfg-accent').value = config.accent || '#0ea5e9';
        if (document.getElementById('pdf-cfg-paper'))
          document.getElementById('pdf-cfg-paper').value = config.paper || 'a4';
        if (document.getElementById('pdf-cfg-header-style'))
          document.getElementById('pdf-cfg-header-style').value = config.header_style || 'full';
        if (document.getElementById('pdf-cfg-orientation'))
          document.getElementById('pdf-cfg-orientation').value = config.orientation || 'portrait';
        if (document.getElementById('pdf-cfg-phone'))
          document.getElementById('pdf-cfg-phone').value = config.phone || '';
        if (document.getElementById('pdf-cfg-website'))
          document.getElementById('pdf-cfg-website').value = config.website || '';
        if (document.getElementById('pdf-cfg-address'))
          document.getElementById('pdf-cfg-address').value = config.address || '';
        if (document.getElementById('pdf-cfg-logo-url'))
          document.getElementById('pdf-cfg-logo-url').value = config.logo_url || '';
        if (document.getElementById('pdf-cfg-banner'))
          document.getElementById('pdf-cfg-banner').value = config.banner || '';
        if (document.getElementById('pdf-cfg-footer'))
          document.getElementById('pdf-cfg-footer').value = config.footer || '';
        if (document.getElementById('pdf-cfg-terms'))
          document.getElementById('pdf-cfg-terms').value = config.terms || '';

        if (document.getElementById('pdf-cfg-show-mmk'))
          document.getElementById('pdf-cfg-show-mmk').checked = config.show_mmk !== false;
        if (document.getElementById('pdf-cfg-show-sig'))
          document.getElementById('pdf-cfg-show-sig').checked = config.show_sig !== false;
        if (document.getElementById('pdf-cfg-show-payment'))
          document.getElementById('pdf-cfg-show-payment').checked = config.show_payment !== false;
        if (document.getElementById('pdf-cfg-show-tax'))
          document.getElementById('pdf-cfg-show-tax').checked = !!config.show_tax;
        if (document.getElementById('pdf-cfg-show-terms'))
          document.getElementById('pdf-cfg-show-terms').checked = config.show_terms !== false;
        if (document.getElementById('pdf-cfg-show-address'))
          document.getElementById('pdf-cfg-show-address').checked = config.show_address !== false;
        if (document.getElementById('pdf-cfg-show-banner-note'))
          document.getElementById('pdf-cfg-show-banner-note').checked = !!config.show_banner_note;
        if (document.getElementById('pdf-cfg-show-itemcode'))
          document.getElementById('pdf-cfg-show-itemcode').checked = config.show_itemcode !== false;
...
// ─────────────────────────────────────────────────────────────────────
// 🛡️ ROLE-BASED ACCESS CONTROL (RBAC) CONTROLLER
// ─────────────────────────────────────────────────────────────────────
window.globalRolesList = [];
...
window.loadRbacSettings = async function () {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (newRoleSel) {
        newRoleSel.innerHTML = window.globalRolesList
          .map((r) => `<option value="${r.name}" class="bg-slate-900">${r.name}</option>`)
          .join('');
      }
      if (editRoleSel) {
        editRoleSel.innerHTML = window.globalRolesList
          .map((r) => `<option value="${r.name}" class="bg-slate-900">${r.name}</option>`)
          .join('');
      }
      if (filterRoleSel) {
        const currentVal = filterRoleSel.value;
        filterRoleSel.innerHTML =
      ...
      if (rolesListDiv) {
        rolesListDiv.innerHTML = window.globalRolesList
          .map((r) => {
            const activeClass =
              window.rbacActiveRole && window.rbacActiveRole.name === r.name
...
window.renderRbacMatrix = function () {
  ...
  if (
    window.rbacActiveRole.name === 'Admin' ||
    window.rbacActiveRole.name === 'Sales' ||
    window.rbacActiveRole.name === 'Technician'
  ) {
                    ...
                    <tr class="border-b border-white/5 hover:bg-white/5 transition align-middle">
                        ...
                        <td class="py-3 px-3 text-center">
                            <label class="inline-flex items-center cursor-pointer">
                                ...
                                <span class="text-[10px] text-emerald-400 ml-1.5 font-bold">Write 🟢</span>
                            </label>
                        </td>
                        <td class="py-3 px-3 text-center">
                            <label class="inline-flex items-center cursor-pointer">
                                ...
                                <span class="text-[10px] text-amber-400 ml-1.5 font-bold">Read 🟡</span>
                            </label>
                        </td>
                        <td class="py-3 px-3 text-center">
                            <label class="inline-flex items-center cursor-pointer">
                                ...
                                <span class="text-[10px] text-rose-400 ml-1.5 font-bold">None 🔴</span>
                            </label>
                        </td>
                    </tr>
                `;
...
window.deleteActiveRbacRole = async function () {
  ...
  if (
    !confirm(
      `Are you sure you want to delete the role "${roleName}"? Any users assigned to this role will lose their custom permissions.`
    )
  ) {
    return;
  }
...
// ── Generic Excel Column Filters Utility ──
window.initExcelTableFilters = function (tableId) {
  ...
  headers.forEach((th, colIndex) => {
    ...
    btn.type = 'button';
    btn.className =
      'excel-filter-btn ml-1.5 inline-flex items-center text-slate-500 hover:text-amber-500 focus:outline-none transition-all';
...
function openExcelFilterPopover(tableId, colIndex, th, btn) {
  ...
  const rows = Array.from(
    tbody.querySelectorAll('tr:not([id^="serials-row"]):not(.empty-state-row)')
  );
  ...
  popover.className =
    'fixed z-50 w-56 rounded-xl border border-white/10 p-3 shadow-2xl space-y-2 text-left';
  popover.style.background =
    'linear-gradient(135deg, rgba(20, 20, 28, 0.98) 0%, rgba(12, 12, 18, 0.98) 100%)';
  popover.style.backdropFilter = 'blur(16px)';

  const headerTitle = th.textContent.replace(/[▼▲\s]/g, '').trim();

  let itemsHtml = sortedValues
    .map((val) => {
      const isChecked =
        !table.activeExcelFilters[colIndex] || table.activeExcelFilters[colIndex].has(val);
      return `
...
// ── Export complete inventory lists directly from memory ──
    ...
    if (type === 'pricing') {
      ...
      data = activeBatchesList.map((b) => {
        ...
        return {
          ...
          SKU: b.item_code,
          'Device Model':
            activeCatalogList.find((c) => c.item_code === b.item_code)?.item_name || b.item_code,
          Category: b.category || '',
...
// ── Generic Table Excel Filter Application ──
