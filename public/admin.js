...
window.fetch = async function (url, options = {}) {
  if (
    ...
    (url.includes('/api/admin/') ||
      url.includes('/api/landing-page') ||
      url.includes('/api/jobs/schedule') ||
      url.includes('/api/jobs/update'))
  ) {
...
function populateMasterDropdowns() {
  ...
  if (catalogUmEl)
    catalogUmEl.innerHTML = '<option value="">â€” Select Unit â€”</option>' + unitOpts;
}
...
window.openEditClient = function (id) {
  ...
  document.getElementById('client-form-title').innerHTML =
    '<span class="text-amber-500">âœï¸</span> Edit Client Profile';
...
window.resetClientForm = function () {
  ...
  document.getElementById('client-form-title').innerHTML =
    '<span class="text-amber-500">ðŸ¢</span> Provision Client Profile';
...
function initCopilotMap() {
  ...
  L.marker([hq.lat, hq.lng])
    .addTo(copilotMap)
    .bindPopup('ðŸ« <strong>AwesomeMyanmar Office (Start/End)</strong>')
    .openPopup();
