...
(function () {
    ...
    window.makeSearchable = function (selectEl, opts) {
        ...
        wrapper.style.cssText = isFullWidth ? 'position:relative;display:block;width:100%;' : 'position:relative;display:inline-block;min-width:' + (selectEl.offsetWidth || 180) + 'px;';
        ...
        chevron.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="width:9px;height:9px;display:block;"><path d="M6 9l6 6 6-6"/></svg>';
        chevron.style.cssText = 'position:absolute;right:0.5rem;top:50%;transform:translateY(-50%);color:#64748b;display:flex;align-items:center;cursor:pointer;transition:transform 0.15s;';
        ...
        function buildList(q) {
            ...
            if (count === 0) {
                ...
                empty.style.cssText = 'padding:0.5rem 0.75rem;font-size:0.68rem;color:#475569;font-style:italic;';
        ...
        input.addEventListener('keydown', function (e) {
            ...
            if (e.key === 'ArrowDown') { e.preventDefault(); if (dd.style.display==='none') open(); moveFocus('down'); return; }
            if (e.key === 'ArrowUp')   { e.preventDefault(); moveFocus('up'); return; }
            if (e.key === 'Enter')     { e.preventDefault(); var f = dd.querySelector('.ss-focused'); if (f) commit(f.dataset.value, f.dataset.label); return; }
            if (e.key === 'Tab')       { close(); }
        });

        chevron.addEventListener('mousedown', function (e) {
            e.preventDefault();
            if (dd.style.display === 'none') { input.focus(); } else { close(); }
        });
        ...
        selectEl._ss = {
            ...
            close:    close,
            sync:     syncFromNative,
            rebuild:  function () { buildList(input.value); },
            setValue: function (v) { selectEl.value = v; syncFromNative(); }
        };
    };
