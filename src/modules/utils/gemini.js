...
export async function fetchGeminiWithFallback(apiKey, payloadBody, model = "gemini-2.5-flash") {
  ...
  try {
    ...
    const isGeoBlocked = errMsg.toLowerCase().includes("location") || 
  ...
  try {
    ...
    throw new Error(`Both Gemini endpoints failed. Primary error: ${lastError ? lastError.message : "none"}. Proxy error: ${err.message}`);
  }
}