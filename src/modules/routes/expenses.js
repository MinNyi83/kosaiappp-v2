...
function register(router, env) {
  ...
  router.get("/api/expenses", async (request) => {
    try {
      ...
      let query = "SELECT e.*, t.name as submitted_by_name FROM expenses e LEFT JOIN technicians t ON e.submitted_by = t.id WHERE 1=1";
      const params = [];
      let countQuery = "SELECT COUNT(*) as total FROM expenses WHERE 1=1";
      const countParams = [];

      if (category) { query += " AND e.category = ?"; params.push(category); countQuery += " AND category = ?"; countParams.push(category); }
      if (dateFrom) { query += " AND e.expense_date >= ?"; params.push(dateFrom); countQuery += " AND expense_date >= ?"; countParams.push(dateFrom); }
      if (dateTo) { query += " AND e.expense_date <= ?"; params.push(dateTo); countQuery += " AND expense_date <= ?"; countParams.push(dateTo); }
      if (status) { query += " AND e.status = ?"; params.push(status); countQuery += " AND status = ?"; countParams.push(status); }
      if (user.role !== "admin") { query += " AND e.submitted_by = ?"; params.push(user.id); countQuery += " AND submitted_by = ?"; countParams.push(user.id); }
  ...
  router.post("/api/expenses", async (request) => {
    try {
      ...
      await db
        ...
        .bind(id, amount, category, description || null, expense_date || new Date().toISOString().split("T")[0], receipt_url || null, user.id)
        .run();
  ...
  router.put("/api/expenses/:id/approve", async (request, params) => {
    try {
      ...
      await db
        .prepare("UPDATE expenses SET status = 'approved', approved_by = ?, approved_at = datetime('now') WHERE id = ?")
  ...
  router.put("/api/expenses/:id/reject", async (request, params) => {
    try {
      ...
      await db
        .prepare("UPDATE expenses SET status = 'rejected', rejection_reason = ?, reviewed_by = ?, reviewed_at = datetime('now') WHERE id = ?")
