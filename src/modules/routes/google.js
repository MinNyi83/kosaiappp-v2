...
function register(router, env) {
  ...
  router.post("/api/auth/google", async (request) => {
    try {
      ...
      if (!tech) {
        ...
        await db
          .prepare("INSERT INTO technicians (id, name, email, google_id, role, active) VALUES (?, ?, ?, ?, 'technician', 1)")
  ...
  router.get("/api/auth/google/drive-url", async () => {
    try {
      ...
      url.searchParams.set("scope", "https://www.googleapis.com/auth/drive.file https://www.googleapis.com/auth/drive.readonly");
  ...
  router.get("/api/auth/google/drive-callback", async (request) => {
    try {
      ...
      if (tokens.refresh_token) {
        await db
          .prepare("INSERT INTO system_config (key, value) VALUES ('google_drive_refresh_token', ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value")
