...
function register(router, env) {
  ...
  router.get("/api/rma", async (request) => {
    try {
      ...
      let query = "SELECT r.*, c.name as client_name FROM rma_requests r LEFT JOIN clients c ON r.client_id = c.id WHERE 1=1";
      const params = [];
  ...
  router.post("/api/rma", async (request) => {
    try {
      ...
      const { client_id, serial_number, item_id, issue_description, notes } = await request.json();
      ...
      await db
        ...
        .bind(id, client_id, serial_number || null, item_id || null, issue_description, notes || null, user.id)
        .run();
  ...
  router.put("/api/rma/:id/status", async (request, params) => {
    try {
      ...
      const validStatuses = ["pending", "approved", "received", "repairing", "repaired", "replaced", "rejected", "closed"];
      ...
      await db
        .prepare("UPDATE rma_requests SET status = ?, resolution_notes = ?, resolved_by = ?, resolved_at = datetime('now') WHERE id = ?")
  ...
  router.get("/api/warranty/check", async (request) => {
    try {
      ...
      return success({
        ...
        days_remaining: isInWarranty ? Math.floor((expiryDate - new Date()) / (1000 * 60 * 60 * 24)) : 0,
      });
    } catch (err) {
