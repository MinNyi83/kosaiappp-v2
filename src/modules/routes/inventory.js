...
function register(router, env) {
  ...
  router.post("/api/inventory", async (request) => {
    try {
      ...
      const { name, sku, category, quantity, unit, reorder_level, description, cost_price, selling_price } = body;
      ...
      await db
        ...
        .bind(
          id, name, sku, category || null,
          quantity || 0, unit || "pcs",
          reorder_level || 0, description || null,
  ...
  router.put("/api/inventory/:id", async (request, params) => {
    try {
      ...
      const allowed = ["name", "sku", "category", "quantity", "unit", "reorder_level", "description", "cost_price", "selling_price"];
      const updates = [];
      const values = [];
  ...
  router.post("/api/inventory/:id/adjust", async (request, params) => {
    try {
      ...
      await db
        ...
        .bind(params.id, item.quantity, newQuantity, quantity_change, reason || "manual adjustment", user.id)
        .run();

      return success({ id: params.id, previous_quantity: item.quantity, new_quantity: newQuantity });
    } catch (err) {
  ...
  router.get("/api/inventory/low-stock", async (request) => {
    try {
      ...
      const items = await db
        .prepare("SELECT * FROM inventory WHERE quantity <= reorder_level ORDER BY (quantity * 1.0 / CASE WHEN reorder_level = 0 THEN 1 ELSE reorder_level END) ASC")
        .all();
  ...
  router.get("/api/inventory/categories", async (request) => {
    try {
      ...
      const result = await db
        .prepare("SELECT DISTINCT category FROM inventory WHERE category IS NOT NULL ORDER BY category ASC")
        .all();
