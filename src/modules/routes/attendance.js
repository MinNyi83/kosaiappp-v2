...
function register(router, env) {
  ...
  router.post("/api/attendance/clock-in", async (request) => {
    try {
      ...
      const existing = await db
        .prepare("SELECT id FROM attendance WHERE technician_id = ? AND date = date('now') AND clock_out IS NULL")
        .bind(user.id)
        .first();
  ...
  router.post("/api/attendance/clock-out", async (request) => {
    try {
      ...
      const record = await db
        .prepare("SELECT * FROM attendance WHERE technician_id = ? AND date = date('now') AND clock_out IS NULL")
        .bind(user.id)
        .first();
  ...
  router.get("/api/attendance", async (request) => {
    try {
      ...
      let query = "SELECT a.*, t.name as technician_name FROM attendance a LEFT JOIN technicians t ON a.technician_id = t.id WHERE 1=1";
      const params = [];

      if (dateFrom) { query += " AND a.date >= ?"; params.push(dateFrom); }
      if (dateTo) { query += " AND a.date <= ?"; params.push(dateTo); }
      if (techId) { query += " AND a.technician_id = ?"; params.push(techId); }
      if (user.role !== "admin") { query += " AND a.technician_id = ?"; params.push(user.id); }
  ...
  router.get("/api/attendance/status", async (request) => {
    try {
      ...
      const record = await db
        .prepare("SELECT * FROM attendance WHERE technician_id = ? AND date = date('now') ORDER BY clock_in DESC LIMIT 1")
        .bind(user.id)
        .first();
