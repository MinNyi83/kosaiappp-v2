...
function register(router, env) {
  ...
  router.post("/api/technicians", async (request) => {
    try {
      ...
      const hashedPin = "$sha256$" + saltHex + "$" + hashArray.map(b => b.toString(16).padStart(2, '0')).join('');
  ...
  router.put("/api/technicians/:id", async (request, params) => {
    try {
      ...
      if (role !== undefined && user.role === "admin") { updates.push("role = ?"); values.push(role); }
      if (specialties !== undefined) { updates.push("specialties = ?"); values.push(JSON.stringify(specialties)); }
      if (active !== undefined && user.role === "admin") { updates.push("active = ?"); values.push(active ? 1 : 0); }
  ...
  router.delete("/api/technicians/:id", async (request, params) => {
    try {
      ...
      await db
        .prepare("UPDATE technicians SET active = 0 WHERE id = ?")
        .bind(params.id)
        .run();
  ...
  router.put("/api/technicians/:id/pin", async (request, params) => {
    try {
      ...
      const encoder = new TextEncoder();
      const [_, algo, storedSalt, storedHash] = tech.pin.match(/^\$(sha256)\$([a-f0-9]+)\$([a-f0-9]+)$/) || [];
      if (algo === 'sha256') {
      ...
      const hashedPin = "$sha256$" + newSaltHex + "$" + newHashArray.map(b => b.toString(16).padStart(2, '0')).join('');
