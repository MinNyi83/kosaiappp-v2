...
function register(router, env) {
  ...
  router.get("/api/reports/dashboard", async (request) => {
    try {
      ...
      const [
        ...
        db.prepare("SELECT COUNT(*) as count FROM jobs WHERE date(scheduled_date) = ?").bind(today).first(),
        db.prepare("SELECT COUNT(*) as count FROM jobs WHERE status = 'pending'").first(),
        db.prepare("SELECT COUNT(*) as count FROM jobs WHERE created_at >= ?").bind(startOfMonth).first(),
        db.prepare("SELECT COALESCE(SUM(amount), 0) as total FROM invoices WHERE status = 'paid' AND paid_at >= ?").bind(startOfMonth).first(),
        db.prepare("SELECT t.name, COUNT(j.id) as job_count FROM jobs j JOIN technicians t ON j.assigned_to = t.id WHERE j.status = 'completed' AND j.updated_at >= ? GROUP BY j.assigned_to ORDER BY job_count DESC LIMIT 5").bind(startOfMonth).all(),
        db.prepare("SELECT j.*, c.name as client_name FROM jobs j LEFT JOIN clients c ON j.client_id = c.id ORDER BY j.created_at DESC LIMIT 10").all(),
  ...
  router.get("/api/reports/jobs", async (request) => {
    try {
      ...
      switch (groupBy) {
        case "technician":
          query = "SELECT t.name as group_key, COUNT(j.id) as total, SUM(CASE WHEN j.status = 'completed' THEN 1 ELSE 0 END) as completed FROM jobs j RIGHT JOIN technicians t ON j.assigned_to = t.id WHERE 1=1";
          if (dateFrom) { query += " AND j.created_at >= ?"; params.push(dateFrom); }
          if (dateTo) { query += " AND j.created_at <= ?"; params.push(dateTo); }
          query += " GROUP BY t.name ORDER BY total DESC";
          break;
        case "date":
          query = "SELECT date(created_at) as group_key, COUNT(*) as total, SUM(CASE WHEN status = 'completed' THEN 1 ELSE 0 END) as completed FROM jobs WHERE 1=1";
    ...
    try {
      ...
      switch (type) {
        ...
        case "inventory":
          ...
          break;
        case "expenses":
          data = await db.prepare("SELECT e.*, t.name as submitted_by_name FROM expenses e LEFT JOIN technicians t ON e.submitted_by = t.id ORDER BY e.expense_date DESC").all();
          break;
        default: // jobs
          data = await db.prepare("SELECT j.*, c.name as client_name, t.name as technician_name FROM jobs j LEFT JOIN clients c ON j.client_id = c.id LEFT JOIN technicians t ON j.assigned_to = t.id ORDER BY j.created_at DESC").all();
      }
