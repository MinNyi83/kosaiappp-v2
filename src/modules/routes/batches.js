...
function register(router, env) {
  ...
  router.get("/api/batches", async (request) => {
    try {
      ...
      let query = "SELECT b.*, i.name as item_name, i.sku FROM batches b LEFT JOIN inventory i ON b.item_id = i.id WHERE 1=1";
      const params = [];

      if (itemId) { query += " AND b.item_id = ?"; params.push(itemId); }
      query += " ORDER BY b.created_at DESC";

      const result = await db.prepare(query).bind(...params).all();

      // Attach serial count to each batch
      const batches = await Promise.all(result.results.map(async (batch) => {
        const serialCount = await db
          .prepare("SELECT COUNT(*) as count, status FROM serial_numbers WHERE batch_id = ? GROUP BY status")
          .bind(batch.id)
          .all();
  ...
  router.post("/api/batches", async (request) => {
    try {
      ...
      const { item_id, quantity, serials, supplier, cost_price, notes } = await request.json();
      ...
      if (serials && serials.length > 0) {
        ...
        for (const serial of serials) {
          const serialId = "SRL-" + Date.now().toString(36).toUpperCase() + Math.random().toString(36).slice(2, 6);
  ...
  router.get("/api/serials", async (request) => {
    try {
      ...
      let query = "SELECT s.*, i.name as item_name, b.batch_number FROM serial_numbers s LEFT JOIN inventory i ON s.item_id = i.id LEFT JOIN batches b ON s.batch_id = b.id WHERE 1=1";
      const params = [];

      if (status) { query += " AND s.status = ?"; params.push(status); }
      if (itemId) { query += " AND s.item_id = ?"; params.push(itemId); }
      if (search) { query += " AND s.serial_number LIKE ?"; params.push(`%${search}%`); }
