...
function register(router, env) {
  ...
  router.post("/api/telegram/send", async (request) => {
    try {
      ...
      const result = await sendTelegramMessage(env, chat_id, text, parse_mode || "HTML");
...
async function handleCommand(chatId, command, from, db, env) {
  const cmds = {
    ...
    "/help": getHelpText(),
    "/jobs": async () => {
      const tech = await db.prepare("SELECT id FROM technicians WHERE telegram_id = ?").bind(from.id).first();
      if (!tech) return "You are not registered as a technician. Contact your admin.";
      const jobs = await db.prepare("SELECT id, title, status, priority FROM jobs WHERE assigned_to = ? AND status IN ('pending', 'assigned', 'in_progress') ORDER BY priority DESC, scheduled_date ASC LIMIT 5").bind(tech.id).all();
    ...
    "/status": async () => {
      const tech = await db.prepare("SELECT id, name FROM technicians WHERE telegram_id = ?").bind(from.id).first();
      if (!tech) return "Not registered.";
      const att = await db.prepare("SELECT * FROM attendance WHERE technician_id = ? AND date = date('now') AND clock_out IS NULL").bind(tech.id).first();
  ...
  await sendTelegramMessage(env, chatId, reply, "Markdown");
}
...
async function handleCallbackQuery(chatId, data, from, db, env) {
  ...
  switch (action) {
    case "accept_job":
      const tech = await db.prepare("SELECT id FROM technicians WHERE telegram_id = ?").bind(from.id).first();
      if (tech) {
        await db.prepare("UPDATE jobs SET status = 'assigned', assigned_to = ? WHERE id = ?").bind(tech.id, jobId).run();
...
async function handleJobCreation(chatId, text, from, db, env) {
  ...
  const tech = await db.prepare("SELECT id FROM technicians WHERE telegram_id = ?").bind(from.id).first();
  if (!tech) {
    return sendTelegramMessage(env, chatId, "You are not registered as a technician. Contact your admin.");
  }

  const id = "JOB-" + Date.now().toString(36).toUpperCase();
  await db
    .prepare("INSERT INTO jobs (id, title, description, status, created_by, notes) VALUES (?, ?, ?, 'pending', ?, ?)")
    .bind(id, text.substring(0, 100), text, tech.id, `Created via Telegram by ${from.first_name || from.id}`)
    .run();

  await sendTelegramMessage(env, chatId, `✅ Job created: #${id}\n\nTitle: ${text.substring(0, 100)}`);
}
