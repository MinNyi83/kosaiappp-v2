...
function register(router, env) {
  ...
  router.post("/api/admin/landing", async (request) => {
    try {
      ...
      const { section_key, title, subtitle, content, image_url, sort_order, active } = await request.json();
      ...
      await db
        ...
        .bind(id, section_key, title, subtitle || null, content || null, image_url || null, sort_order || 0, active !== undefined ? (active ? 1 : 0) : 1)
        .run();
  ...
  router.put("/api/admin/landing/:id", async (request, params) => {
    try {
      ...
      const allowed = ["section_key", "title", "subtitle", "content", "image_url", "sort_order", "active"];
      const updates = [];
      const values = [];
