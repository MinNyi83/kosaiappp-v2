...
function register(router, env) {
  ...
  router.get('/api/jobs', async (request) => {
    try {
      ...
      let query =
        'SELECT j.*, c.name as client_name, c.phone as client_phone, c.address as client_address FROM jobs j LEFT JOIN clients c ON j.client_id = c.id WHERE 1=1';
      const params = [];
  ...
  router.get('/api/jobs/active', async (request) => {
    try {
      ...
      let query =
        'SELECT j.*, c.name as client_name, c.phone as client_phone, c.address as client_address FROM jobs j LEFT JOIN clients c ON j.client_id = c.id WHERE j.status IN (?, ?, ?, ?)';
  ...
  router.get('/api/jobs/:id', async (request, params) => {
    try {
      ...
      const job = await db
        .prepare(
          'SELECT j.*, c.name as client_name, c.phone as client_phone, c.address as client_address FROM jobs j LEFT JOIN clients c ON j.client_id = c.id WHERE j.id = ?'
        )
        .bind(params.id)
        .first();
  ...
  router.put('/api/jobs/:id', async (request, params) => {
    try {
      ...
      const body = await request.json();
      const existing = await db.prepare('SELECT * FROM jobs WHERE id = ?').bind(params.id).first();
  ...
  router.delete('/api/jobs/:id', async (request, params) => {
    try {
      ...
      const existing = await db.prepare('SELECT id FROM jobs WHERE id = ?').bind(params.id).first();
      if (!existing) return error('Job not found', 404);
  ...
  router.get('/api/jobs/calendar', async (request) => {
    try {
      ...
      let query =
        'SELECT j.id, j.title, j.status, j.priority, j.scheduled_date, j.assigned_to, t.name as technician_name, c.name as client_name FROM jobs j LEFT JOIN technicians t ON j.assigned_to = t.id LEFT JOIN clients c ON j.client_id = c.id WHERE j.scheduled_date >= ?';
