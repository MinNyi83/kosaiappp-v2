...
function register(router, env) {
  ...
  router.post("/api/public/contact", async (request) => {
    try {
      ...
      await db
        .prepare("INSERT INTO contact_submissions (id, name, email, phone, message) VALUES (?, ?, ?, ?, ?)")
  ...
  router.get("/api/public/exchange-rate", async (request) => {
    try {
      ...
      const cached = await db
        .prepare("SELECT * FROM exchange_rates WHERE base_currency = ? AND target_currency = ? AND updated_at > datetime('now', '-1 hour')")
        .bind(base, target)
        .first();
      ...
      try {
        ...
        if (rate) {
          // Cache it
          await db
            .prepare("INSERT INTO exchange_rates (base_currency, target_currency, rate) VALUES (?, ?, ?) ON CONFLICT(base_currency, target_currency) DO UPDATE SET rate = excluded.rate, updated_at = datetime('now')")
  ...
  router.get("/api/public/serials", async (request) => {
    try {
      ...
      const item = await db
        .prepare("SELECT i.name, i.sku, b.batch_number, s.serial_number, s.status FROM serial_numbers s JOIN inventory i ON s.item_id = i.id LEFT JOIN batches b ON s.batch_id = b.id WHERE s.serial_number = ?")
        .bind(serial)
        .first();
