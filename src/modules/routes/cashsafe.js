...
function register(router, env) {
  ...
  router.get("/api/cash-safe/balance", async (request) => {
    try {
      ...
      const balance = await db
        .prepare("SELECT COALESCE(SUM(CASE WHEN type = 'deposit' THEN amount ELSE -amount END), 0) as balance FROM cash_transactions")
        .first();

      const todayTotal = await db
        .prepare("SELECT COALESCE(SUM(amount), 0) as total FROM cash_transactions WHERE date(created_at) = date('now')")
        .first();
  ...
  router.post("/api/cash-safe/deposit", async (request) => {
    try {
      ...
      await db
        .prepare("INSERT INTO cash_transactions (id, type, amount, notes, created_by) VALUES (?, 'deposit', ?, ?, ?)")
  ...
  router.post("/api/cash-safe/withdraw", async (request) => {
    try {
      ...
      const id = "CSH-" + Date.now().toString(36).toUpperCase();
      await db
        .prepare("INSERT INTO cash_transactions (id, type, amount, notes, created_by) VALUES (?, 'withdraw', ?, ?, ?)")
        .bind(id, amount, notes || null, user.id)
        .run();
  ...
  router.get("/api/cash-safe/transactions", async (request) => {
    try {
      ...
      let query = "SELECT ct.*, t.name as created_by_name FROM cash_transactions ct LEFT JOIN technicians t ON ct.created_by = t.id WHERE 1=1";
      const params = [];

      if (dateFrom) { query += " AND ct.created_at >= ?"; params.push(dateFrom); }
      if (dateTo) { query += " AND ct.created_at <= ?"; params.push(dateTo); }
