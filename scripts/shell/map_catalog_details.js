...
async function main() {
  ...
  const catMap = {};
  cats.forEach((c) => {
    catMap[c.category_id.padStart(2, '0')] = c.category_description.trim();
  });
  ...
  const subMap = {};
  subs.forEach((s) => {
    subMap[s.sub_category_id.padStart(3, '0')] = s.sub_category_description.trim();
  });
  ...
  const brandMap = {};
  brands.forEach((b) => {
    brandMap[b.brand_id.padStart(2, '0')] = b.brand_description.trim();
  });

  // Load entire stock codes lookup from SQLite
  const sqliteStocks = sqliteDb
    .prepare('SELECT stock_id, category_id, sub_category_id, brand_id, stocking_um FROM stock')
    .all();
  const lookup = {};
  sqliteStocks.forEach((s) => {
    if (s.stock_id) {
      lookup[s.stock_id.trim().toUpperCase()] = {
        ...
        subCategoryName: s.sub_category_id
          ? subMap[String(s.sub_category_id).trim().padStart(3, '0')]
          : null,
