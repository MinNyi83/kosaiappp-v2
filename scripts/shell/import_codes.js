...
async function main() {
  ...
  console.log(
    `D1 state: ${d1Cats.length} cats, ${d1Subs.length} sub-cats, ${d1Brands.length} brands`
  );
  ...
  for (const c of excelCategories) {
    // Post to /add
    const res = await apiFetch('/api/admin/inventory/categories/add', 'POST', {
      name: c.name,
      code: c.code,
    });
  ...
  for (const b of excelBrands) {
    const res = await apiFetch('/api/admin/inventory/brands/add', 'POST', {
      name: b.name,
      code: b.code,
    });
    process.stdout.write('.');
  ...
  categoriesList.forEach((c) => {
    sqliteCatById[c.category_id.padStart(2, '0')] = c.category_description.trim();
  });
  const sqliteSubById = {};
  subcategoriesList.forEach((s) => {
    sqliteSubById[s.sub_category_id.padStart(3, '0')] = s.sub_category_description.trim();
  });
  ...
  for (const s of excelSubCategories) {
    ...
    const existingSub = d1Subs.find(
      (ds) => ds.name.trim().toLowerCase() === s.name.trim().toLowerCase()
    );
    if (existingSub) {
