...
function apiFetch(path, method = 'GET', body = null) {
    return new Promise((resolve, reject) => {
        ...
        const opts = {
            hostname: '127.0.0.1', port: 8787, path, method,
            headers: {
...
async function main() {
    ...
    const catById = {};
    categories.forEach(c => { catById[c.category_id.padStart(2, '0')] = c.category_description.trim(); });

    // sub_category_id (padded) → description
    const subById = {};
    subcategories.forEach(s => { subById[s.sub_category_id.padStart(3, '0')] = s.sub_category_description.trim(); });

    // Count category votes per sub-category from stock data
    const subCatFreq = {};
    const stocks = db.prepare('SELECT category_id, sub_category_id FROM stock WHERE sub_category_id IS NOT NULL AND category_id IS NOT NULL').all();
    ...
    db.close();
    console.log(`✅ Loaded ${Object.keys(subNameToParentCatName).length} sub-cat→category mappings from SQLite\n`);
    ...
    const d1CatNameToId = {};
    d1Cats.forEach(c => { d1CatNameToId[c.name.trim().toLowerCase()] = c.id; });
    ...
    for (const sub of d1SubCats) {
        ...
        toUpdate.push({ id: sub.id, name: sub.name, category_id: parentCatId, category_name: parentCatName });
    }
    ...
    if (noMatch.length > 0) {
        console.log(`   Unmatched: ${noMatch.slice(0, 10).join(', ')}${noMatch.length > 10 ? '...' : ''}`);
    }
    console.log('');
