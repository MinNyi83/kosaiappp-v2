...
const excelStockItems = stockRows.slice(headerIdx + 1)
    .filter(r => r[0] && typeof r[0] === 'string' && r[0].trim() !== '' && r[0] !== 'Stock_ID')
    .map(r => {
        const obj = {};
        stockHeaders.forEach((h, i) => { if (h) obj[h] = r[i]; });
        return obj;
    })
...
async function main() {
    ...
    console.log('╚══════════════════════════════════════════════════════╝\n');
    console.log(`📊 Excel Data: ${excelCategories.length} categories, ${excelSubCategories.length} sub-cats, ${excelBrands.length} brands, ${excelStockItems.length} stock items\n`);
    ...
    const existingUnitAbbrevs = new Set(existingUnits.map(u => u.abbreviation.trim().toLowerCase()));
    const existingStockCodes = new Set(existingStock.map(s => s.item_code.trim().toUpperCase()));

    console.log(`   DB has: ${existingCats.length} cats, ${existingSubs.length} sub-cats, ${existingBrands.length} brands, ${existingUnits.length} units, ${existingStock.length} stock items\n`);

    // ── 2. Import Categories ─────────────────────────────────────────────────
    const missingCats = excelCategories.filter(c => !existingCatNames.has(c.name.toLowerCase()));
    console.log(`📁 Categories: ${missingCats.length} new to import (${excelCategories.length - missingCats.length} already exist)`);
    ...
    for (const cat of missingCats) {
        ...
        if (r.status === 200 || r.status === 201) { catOk++; process.stdout.write('.'); }
        else { catFail++; console.log(`\n   ❌ Failed: ${cat.name} — ${JSON.stringify(r.body)}`); }
        await sleep(30);
    }
    ...
    const catNameToId = {};
    allCats.forEach(c => { catNameToId[c.name.trim().toLowerCase()] = c.id; });
    ...
    const missingSubCats = excelSubCategories.filter(s => !existingSubNames.has(s.name.toLowerCase()));
    ...
    for (const sub of missingSubCats) {
        const r = await apiFetch('/api/admin/inventory/sub-categories/add', 'POST', { name: sub.name, category_id: null });
        if (r.status === 200 || r.status === 201) { subOk++; process.stdout.write('.'); }
        else { subFail++; console.log(`\n   ❌ Failed: ${sub.name}`); }
        await sleep(30);
    }
    ...
    let brandOk = 0, brandFail = 0;
    for (const brand of missingBrands) {
        const r = await apiFetch('/api/admin/inventory/brands/add', 'POST', { name: brand.name });
        if (r.status === 200 || r.status === 201) { brandOk++; process.stdout.write('.'); }
        else { brandFail++; console.log(`\n   ❌ Failed: ${brand.name}`); }
        await sleep(30);
    }
    ...
    const missingUnits = unitsToImport.filter(u => !existingUnitAbbrevs.has(u.abbreviation.toLowerCase()));
    console.log(`\n📏 Stock Units: ${missingUnits.length} new to import (${unitsToImport.length - missingUnits.length} already exist)`);
    let unitOk = 0, unitFail = 0;
    for (const unit of missingUnits) {
        const r = await apiFetch('/api/admin/inventory/units/add', 'POST', { name: unit.name, abbreviation: unit.abbreviation });
        if (r.status === 200 || r.status === 201) { unitOk++; process.stdout.write('.'); }
        else { unitFail++; console.log(`\n   ❌ Failed: ${unit.abbreviation}`); }
        await sleep(30);
    }
    ...
    let stockOk = 0, stockFail = 0, stockSkip = 0;
    const batchSize = 20;
    const failedItems = [];
    ...
    for (let i = 0; i < missingStock.length; i++) {
        ...
        const sub_category_id = String(s.Sub_Category_ID || '').replace(/-/g, '').trim() || null;
        ...
        if (!item_code || !item_name) { stockSkip++; continue; }
        ...
        if (r.status === 200 || r.status === 201) {
            stockOk++;
            if (stockOk % batchSize === 0) process.stdout.write(`\r   Progress: ${stockOk}/${missingStock.length} imported...`);
        } else {
            stockFail++;
    ...
    console.log(`║  Failed/Skipped      : ${String(catFail+subFail+brandFail+unitFail+stockFail+stockSkip).padEnd(27)}║`);
    console.log('╚══════════════════════════════════════════════════════╝\n');
